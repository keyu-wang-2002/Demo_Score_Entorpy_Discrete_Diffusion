...
To address these limitations, SEDD introduces the **Score Entropy Loss** as a replacement for the MSE objective:  $( s_\theta(x_t, t)_y - \frac{p_t(y)}{p_t(x)} )^2$ $s_\theta(x_t, t)_y - \frac{p_t(y)}{p_t(x)} log s_\theta(x_t, t)_y$
